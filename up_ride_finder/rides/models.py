...
class Ride(models.Model):
    destination = models.CharField(
        _('ZIP code of final destination'), max_length=5, validators=[valid_zip])
    origin = models.CharField(_('ZIP code of origin'),
                              max_length=5, validators=[valid_zip])
    available_seats = models.IntegerField(
        _('Maximum number of passengers'), validators=[MinValueValidator(1)])
    cost = models.CharField(
        _('Approx. cost the driver expects riders to pay.'), max_length=255)
    when = models.DateTimeField(
        _('Approximate departure date and time'), validators=[future_date])
    ...
    dest_name = models.CharField(
        _('Friendly name of final destination'), max_length=255, blank=True)
    origin_name = models.CharField(
        _('Friendly name of origin'), max_length=255, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.dest_name or not self.origin_name:
            gmaps = googlemaps.Client(key=getattr(
                settings, 'GOOGLE_MAPS_GEOCODING_KEY', None), timeout=5)
...
class RideRequest(models.Model):
    ...
    def __str__(self):
        return "User {}'s {} request for ride {}".format(self.requester_id, self.get_status_display(), self.ride_id)
